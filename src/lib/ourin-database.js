...
class Database {
  ...
  migrateFromOldPath() {
    ...
    console.log(
      `[Database] Migrasi ${oldFiles.length} file dari src/database/ → ${path.relative(process.cwd(), this.dbPath)}/`,
    );
    this.ensureDir();
    ...
    for (const file of oldFiles) {
      ...
      try {
        ...
        console.error(`[Database] Gagal migrasi ${file}:`, e.message);
      }
    }
    console.log("[Database] Migrasi path selesai");
  }
  ...
  async init() {
    try {
      ...
      if (lastDefault !== currentDefault || lastPremium !== currentPremium) {
        ...
        if (synced > 0) {
          console.log(
            `[Database] Energi sync: ${synced} user di-update (default: ${currentDefault}, premium: ${currentPremium})`,
          );
        }
      }
      ...
      this.ready = true;
      console.log(
        "[Database] Multi-file database siap (debounced write setiap 5s)",
      );
      return this;
    } catch (error) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (!content || content === "" || content === "{}") {
        ...
        try {
          ...
          console.log(
            `[Database] ${fileName} rusak, backup disimpan: ${backup}`,
          );
        }
      }
    } else {
  ...
  async migrateFromSingleFile() {
    ...
    try {
      ...
      console.log(
        `[Database] Migrasi dari energis.json selesai, backup: ${path.basename(backupPath)}`,
      );
    } catch (e) {
      console.error(`[Database] Gagal migrasi energis.json: ${e.message}`);
    }
  }
