...
async function serialize(sock, msg, store = {}) {
  ...
  if (!m.isGroup && !m.isNewsletter && (isLid(m.chat) || isLidConverted(m.chat))) {
    if (remoteJidAlt && !isLid(remoteJidAlt) && !isLidConverted(remoteJidAlt)) {
      const lidKey = m.chat.endsWith("@lid") ? m.chat : m.chat.replace("@s.whatsapp.net", "@lid");
  ...
  if (!senderJid || isLid(senderJid) || isLidConverted(senderJid)) {
    ...
    if (altJid && !isLid(altJid) && !isLidConverted(altJid)) {
      if (senderJid) {
        const lidKey = senderJid.endsWith("@lid") ? senderJid : senderJid.replace("@s.whatsapp.net", "@lid");
  ...
  m.pushName = msg.pushName || (m.isNewsletter ? "Channel" : "Unknown");
  m.isBot = m.fromMe;
    ...
    try {
      ...
      m.isAdmin = m.groupMembers.some((p) => {
        ...
        return (
          pNum === senderNum ||
          pLidNum === senderNum ||
          (pNum.length >= 8 && senderNum.length >= 8 && (pNum.endsWith(senderNum) || senderNum.endsWith(pNum)))
        );
      });
      ...
      m.isBotAdmin = m.groupMembers.some((p) => {
        ...
        return (
          pNum === botNum ||
          (pNum.length >= 8 && botNum.length >= 8 && (pNum.endsWith(botNum) || botNum.endsWith(pNum)))
        );
      });
