...
const pluginConfig = {
    ...
    alias: ['aaw', 'autoanime'],
    category: 'anime',
    description: 'Subida automática de anime y donghua en emisión desde winbu.net (720p Pixeldrain)',
...
async function handler(m, { sock, args }) {
    ...
    switch (sub) {
        case 'start': {
            ...
            if (groups.length === 0) {
                return m.reply(
                    `❌ ¡No hay grupos objetivo configurados!\n\n` +
                    `> Agrega un grupo primero:\n` +
                    `> \`${m.prefix}autoanimewinbu addgrup\` (dentro del grupo)\n` +
        ...
        case 'check': {
            ...
            await m.reply('🔍 Buscando nuevos animes...')
            try {
                await runCheck()
        ...
        case 'reset': {
            ...
            saveSent(new Set())
            return m.reply(`✅ ¡Reset exitoso! *${count}* episodios eliminados del historial.\n> Todos los episodios podrán enviarse de nuevo.`)
        }
        ...
        case 'addgroup': {
            ...
            if (!grupId || !grupId.includes('@g.us')) {
                return m.reply(
                    ...
                    `> Úsalo dentro de un grupo o escribe:\n` +
                    `> \`${m.prefix}autoanimewinbu addgrup 120363xxx@g.us\``
                )
            }
        ...
        case 'delgroup': {
            ...
            if (idx === -1) {
                return m.reply(`❌ El grupo no se encuentra en la lista`)
            }
        ...
        default: {
            ...
            return sock.sendMessage(m.chat, {
                text: `🎬 *ᴀᴜᴛᴏ ᴀɴɪᴍᴇ ᴡɪɴʙᴜ*\n\n` +
                    ...
                    `> \`${m.prefix}aaw cek\` — Revisión manual ahora\n` +
                    `> \`${m.prefix}aaw list\` — Lista de anime reciente\n` +
                    `> \`${m.prefix}aaw addgrup\` — Añadir grupo objetivo\n` +
                    `> \`${m.prefix}aaw delgrup\` — Eliminar grupo objetivo\n` +
                    `> \`${m.prefix}aaw interval 10\` — Cambiar intervalo\n` +
                    `> \`${m.prefix}aaw reset\` — Resetear historial de envíos`,
                interactiveButtons: [
                    {
                        name: 'quick_reply',
                        buttonParamsJson: JSON.stringify({
