...
async function tiktokDl(url) {
        function formatNumber(integer) {
                ...
                return Number(numb).toLocaleString('es-AR').replace(/,/g, '.')
        }
...
const pluginConfig = {
    ...
    category: 'download',
    description: 'Descargar video/slides de TikTok sin marca de agua',
    usage: '.tiktok <url>',
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        // Si es un video (duración mayor a 0)
        if (result.durations > 0 && result.duration !== "0 Segundos") {
            let media = await result.data.find(e => e.type == "nowatermark_hd" || e.type == "nowatermark");
            await sock.sendMedia(m.chat, media.url, result.title, m, {
                type: 'video',
                contextInfo: {
                    forwardingScore: 99,
                    isForwarded: true
            ...
            // Si es un carrusel de fotos (slide)
            const album = result.data?.map(zan => ({
                    image: { url: zan.url }
            }))
