...
const pluginConfig = {
  name: "fakestory4",
  alias: ["fstory4", "igstory4", "albumstory", "historiafalsa4"],
  category: "canvas",
  description: "Crea una historia de Instagram falsa con 2 imágenes distintas",
  usage: ".fakestory4 <nombre>",
  example: ".fakestory4 Misaki (responde a un álbum/2 imágenes)",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  const username = m.args.join(" ").trim() || m.pushName || "Usuario";
  m.react("🕕");
  ...
  try {
    ...
    if (!imageTopBuffer) {
      ...
      return m.reply(
        `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ 4*\n\n` +
          `> ¡Envía o responde a 1-2 imágenes!\n\n` +
          `> Formato: \`${m.prefix}fakestory4 <nombre>\`\n` +
          `> Ejemplo: \`${m.prefix}fakestory4 Misaki\`\n\n` +
          `> Tips: Envía una imagen y responde a otra para usar 2 fotos diferentes.`,
      );
    }
