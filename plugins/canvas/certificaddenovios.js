...
const pluginConfig = {
    name: 'certamor',
    alias: ['certamor', 'certificadonovios', 'certnovios', 'certificadoamor'],
    category: 'canvas',
    description: 'Crea un certificado de amor personalizado en español',
    usage: '.certamor <nombre1> <nombre2>',
    example: '.certamor Luis Maria',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (args.length < 2) {
        return m.reply(
            `💖 *CERTIFICADO DE AMOR*\n\n` +
            `╭┈┈⬡「 📋 *MODO DE USO* 」\n` +
            `┃ ◦ \`${m.prefix}certamor <nombre1> <nombre2>\`\n` +
            `╰┈┈⬡\n\n` +
            `> Ejemplo: \`${m.prefix}certamor Luis Maria\``
        )
    }
    ...
    try {
        ...
        if (!fs.existsSync(bgPath)) {
            throw new Error(`Archivo no encontrado: ${bgPath}`)
        }
        ...
        console.error('[certamor] Error:', error)
        m.react('☢')
